...
plan = json.load(open("survey_plan.json")); plan_days = sum(p.get("ship_days", 0) for p in plan); plan_area = sum(p.get("area_km2", 0) for p in plan); plan_cost = sum(p.get("cost_CAD_M", 0) for p in plan) * 1e6
...
<div class="tcap"><b>Table 4.</b> Temporal holdout by depth. In water shallower than 50 m the mean-depth model reads {min(abs(tdep[(20,50)]['bias']),abs(tdep[(0,20)]['bias'])):.0f}–{max(abs(tdep[(20,50)]['bias']),abs(tdep[(0,20)]['bias'])):.0f} m too deep — the direction that matters for a keel, and the motivation for §5.</div>
...
{("<h3>5.1 Blind inputs and sensitivity</h3><p>Table 6 lets the model see every published sounding around a site at inference; NONNA carries no dates after 2016, so a post-incident survey could be among them. Table 6b removes every sounding within 10 km of each site from the input as well (the hazard model was trained with all tile groups within 0.4° of the sites excluded); a 25 km hole is also shown, at which the site lies outside the 12 km inference envelope and the model returns no estimate. The two denominator choices are then varied: comparison window (10/25/50 km) and the apparently-safe filter (1.5×, 2×, 3× draft on the mean map). Skill: by construction 10%% of apparently-safe cells lie above the 90th percentile, so with the 10 km hole the %d-of-%d result has binomial p = %.4f (all six) and %d of %d, p = %.4f, on the four uncharted-shoal groundings; with all soundings visible, %d of %d (p = %.4f). Precision cannot be bounded from six incidents; corridor-wide %.0f%% of cells exceed P = 5%% and %.0f of %.0f apparently-safe route-km are flagged.</p><div class='tw'><table><thead>%s</thead><tbody>%s</tbody></table></div><div class='tcap'><b>Table 6b.</b> Hazard percentile of the strike site under input holes and denominator choices (window / safe-filter). 10 km hole with the 25 km window and 2× draft is the reference blind number; at 25 km the model has no input within its envelope and abstains.</div>" % (HB["skill"]["mask_10km"]["hits_all"][0], HB["skill"]["mask_10km"]["hits_all"][1], HB["skill"]["mask_10km"]["p_binomial_all"], HB["skill"]["mask_10km"]["hits_uncharted4"][0], HB["skill"]["mask_10km"]["hits_uncharted4"][1], HB["skill"]["mask_10km"]["p_binomial_uncharted4"], HB["skill"]["mask_0km"]["hits_all"][0], HB["skill"]["mask_0km"]["hits_all"][1], HB["skill"]["mask_0km"]["p_binomial_all"], hz["frac_p_gt_5pct"]*100, hz["km_flagged"], hz["km_mean_map_safe"], tr(["Grounding", "all soundings", "10 km hole", "25 km hole", "10 km hole · 10 km window", "10 km hole · 50 km window", "10 km hole · safe 1.5×", "10 km hole · safe 3×"], True), "".join(tr([f"<b>{n}</b>"] + [("—" if (v := HB["sites"][n].get(mk, {}).get("ranks", {}).get(key, {}).get("percentile")) is None else f"{v:.0f}") for mk, key in [("mask_0km","win25km_safe2x"),("mask_10km","win25km_safe2x"),("mask_25km","win25km_safe2x"),("mask_10km","win10km_safe2x"),("mask_10km","win50km_safe2x"),("mask_10km","win25km_safe1.5x"),("mask_10km","win25km_safe3x")]]) for n in ["Thamesborg","Akademik Ioffe","Clipper Adventurer","Hanseatic","Nanny 2012","Nanny 2014"] if n in HB["sites"]))) if HB else ""}
