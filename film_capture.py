...
try:
    with sync_playwright() as pw:
        ...
        tw = time.time(); pg.wait_for_function("() => !!window.__warmDone", timeout=900000); print(f"tiles warmed in {time.time()-tw:.0f}s", flush=True)
        time.sleep(1.0)
...
subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-ss", f"{1.0*A.slow}", "-i", "film/churchill_corridor_raw.mp4", "-loop", "1", "-t", "5", "-framerate", str(A.fps), "-i", "film/endcard.png",
    "-filter_complex", f"[0:v]setpts=PTS/{A.slow},fps={A.fps},fade=t=in:st=0:d=1.2,fade=t=out:st=84.5:d=1.2,setsar=1[a];[1:v]scale={A.w}:{A.h},fade=t=in:st=0:d=1,fade=t=out:st=4:d=1,setsar=1[b];[a][b]concat=n=2:v=1:a=0[v]",
