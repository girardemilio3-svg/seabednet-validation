...
R = 6378137.0; STRIDE = 5; SOUTH = os.environ.get("DA_SOUTH", "0") == "1"   # DA_SOUTH=1: blocks south of 56N, every chart-safe cell (no route filter), separate outputs
SCORE_DIR = "danger_score_south" if SOUTH else "danger_score"; PLAN = "national_plan_v3_south.json" if SOUTH else "national_plan_v3.json"
...
G = GravityPrior(); os.makedirs(SCORE_DIR, exist_ok=True); boxes = {}; ix = {k: i for i, k in enumerate(FEATS)}
for fpath in sorted(glob.glob("hazard_nat_v2/*.npz")):
    ...
    blat = math.degrees(2*math.atan(math.exp((y0+y1)/2/R)) - math.pi/2)
    if SOUTH != (blat < 56): continue
    kk = np.isfinite(Z)
    ...
    rel = safe if SOUTH else safe & ((tree_r.query(q)[0]/scale < 30000) | (tree_c.query(q)[0]/scale < 15000) | (tree_n.query(q)[0]/scale < 30000))
    ...
    np.savez_compressed(f"{SCORE_DIR}/{name}", i=I.astype(np.int16), j=J.astype(np.int16), score=sc.astype(np.float16), bbox3857=bb)
...
json.dump(out, open(PLAN, "w"), indent=1); print("PLAN_V3_DONE", out["total"])
